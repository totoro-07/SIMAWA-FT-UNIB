        ...
        <form action="{{ route('admin.kegiatan.update', $kegiatan->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('jenis_kegiatan') is-invalid @enderror" 
                    ...
                    <option value="Akademik" {{ old('jenis_kegiatan', $kegiatan->jenis_kegiatan) == 'Akademik' ? 'selected' : '' }}>Akademik</option>
                    <option value="Non-Akademik" {{ old('jenis_kegiatan', $kegiatan->jenis_kegiatan) == 'Non-Akademik' ? 'selected' : '' }}>Non-Akademik</option>
                </select>
            ...
            <div class="mb-3">
                ...
                @if($kegiatan->gambar)
                    <small class="text-muted">Gambar saat ini: <a href="{{ asset('storage/' . $kegiatan->gambar) }}" target="_blank">Lihat Gambar</a></small>
                @endif
                @error('gambar')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="status" name="status">
                    <option value="pending" {{ old('status', $kegiatan->status) == 'pending' ? 'selected' : '' }}>Menunggu Verifikasi</option>
                    <option value="approved" {{ old('status', $kegiatan->status) == 'approved' ? 'selected' : '' }}>Terverifikasi</option>
                    <option value="rejected" {{ old('status', $kegiatan->status) == 'rejected' ? 'selected' : '' }}>Ditolak</option>
                </select>                
                @error('status')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
